...
func TestGetUserApiKeyWhenUserNotExists(t *testing.T) {
	...
	user := model.NewUser("api_user_4", "api_user", "", model.NewDefaultUserSettings(), time.Now().Unix(), time.Now().Unix())
	err := database.CreateUser(ctx, user)
	assert.NoError(err)
...
func TestRegeneratesUserApiKey(t *testing.T) {
	...
	user := model.NewUser("api_user_5", "api_user", "", model.NewDefaultUserSettings(), time.Now().Unix(), time.Now().Unix())
	user.ApiKey = &ValidApiKey
...
func TestRevokesUserApiKey(t *testing.T) {
	...
	user := model.NewUser("api_user_6", "api_user", "", model.NewDefaultUserSettings(), time.Now().Unix(), time.Now().Unix())
	user.ApiKey = &ValidApiKey
